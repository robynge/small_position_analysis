...
def analyze_etf(etf_name):
    ...
    graduated_col = f'Graduated to >{CURRENT_RANGE["max"]}%'
    recovered_col = f'Recovered to >{CURRENT_RANGE["max"]}%'

    # Safe counting for empty DataFrames
    graduated_count = len(starters[starters['Outcome'] == 'Graduated to Large']) if len(starters) > 0 else 0
    still_small_count = len(starters[starters['Outcome'] == 'Still Small']) if len(starters) > 0 else 0
    dropped_count = len(starters[starters['Outcome'] == 'Dropped']) if len(starters) > 0 else 0

    recovered_count = len(residuals[residuals['Outcome'] == 'Recovered to Large']) if len(residuals) > 0 else 0
    still_residual_count = len(residuals[residuals['Outcome'] == 'Still Residual']) if len(residuals) > 0 else 0
    residual_dropped_count = len(residuals[residuals['Outcome'] == 'Dropped']) if len(residuals) > 0 else 0

    summary = pd.DataFrame([{
        'ETF': etf_name,
        'Total Starter Positions': len(starters),
        graduated_col: graduated_count,
        'Still in Range': still_small_count,
        'Dropped': dropped_count,
        'Starter Success Rate %': graduated_count / len(starters) * 100 if len(starters) > 0 else 0,
        'Avg Days as Starter': starters['Days as Small Position'].mean() if len(starters) > 0 else 0,
        'Total Residual Positions': len(residuals),
        recovered_col: recovered_count,
        'Still Residual': still_residual_count,
        'Residual Dropped': residual_dropped_count,
        'Residual Recovery Rate %': recovered_count / len(residuals) * 100 if len(residuals) > 0 else 0,
...
def run():
    ...
    for etf in etfs:
        summary, _, _, _ = analyze_etf(etf)
        all_summaries.append(summary)
        print(f"  ✓ {etf}: Starter/Residual analysis")
    ...
    totals = pd.DataFrame([{
        ...
        graduated_col: combined_summary[graduated_col].sum(),
        'Still in Range': combined_summary['Still in Range'].sum(),
        'Dropped': combined_summary['Dropped'].sum(),
        'Starter Success Rate %': combined_summary[graduated_col].sum() / combined_summary['Total Starter Positions'].sum() * 100,
        ...
        recovered_col: combined_summary[recovered_col].sum(),
        'Still Residual': combined_summary['Still Residual'].sum(),
        'Residual Dropped': combined_summary['Residual Dropped'].sum(),
        'Residual Recovery Rate %': combined_summary[recovered_col].sum() / combined_summary['Total Residual Positions'].sum() * 100,
